...
Die hier anfallenden Fehler äußern sich in sechs Permutationen, in welchen immer nur ein String (\texttt{translatable}) übersetzt werden darf und ein weiterer String  (\texttt{non-translatable}) nicht. Sie zeigen sich in \verb|\command[non-translatable]{non-translatable}|, \verb|\command[translatable]{non-translatable}|, \verb|\command[non-translatable]{translatable}|, \verb|\command[translatable]{translatable}|, \verb|\command[non-translatable] translatable|\pdfcomment{bspw.\ \marginpar[translatable] text, benötigt logischerweise den Platz dafür} oder \verb|\command[translatable] translatable| \pdfcomment{bspw.\ \item[translatable] text} und sind anhand spezifischer Kommandos erkennbar.


%% Braucht anderen Titel. 
\paragraph*{Parameter-Unendlichkeit}
Eigene definierte Makros erlauben für eine quasi-beliebige Zahl an Stellen, wo innerhalb von geschwungenen oder eckigen Klammern zu übersetzende Strings zu erwarten sein könnten.% Muss später weiter verfolgt werden.
