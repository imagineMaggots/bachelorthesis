...
% Struktur:
%   Problem > 
%       Wie sieht es vor und nach falscher Übersetzung aus?
%       Warum könnte es auftreten?
%       Wie formuliert man es abstrakter? Geht richtung RegEx
%%%%%




% Dinge, welche sehr schnell in TeX-relevant werden und sich einfach lösen lassen
\subsection{Simple Probleme}
\paragraph*{Textliche Formatierung}% Eigentlich nur einfache \TeX{}-Syntax... die simpelsten Fälle, an welchen reine Übersetzungsprogramme scheitern müssten... Nicht alle Wörter stehen frei (durch Whitespace getrennt)
...
%So ist innerhalb der beispielhaften Zeichenkette vermutlich \verb|\title{| als ein Wort aufgefasst und danach \enquote{My Title} als freistehende Worte, wodurch diese übersetzt werden. Anders ist dies, wenn kein unerwartetes Zeichen (i.e.\ ein von Whitespace Abweichendes) einer Zeichenfolge aus reinen Buchstaben folgt. 
%Google Translate würde beispielsweise für die Zeichenkette \texttt{Hello} korrekterweise das Deutsche \texttt{Hallo} liefern, aber bereits die Präambel von \TeX{}-Dokumenten zeigt, wie \verb|\title{}|, \verb|\author{}| und \verb|\date{}| respektiv zu \verb|\Titel{}|, \verb|\Autorin{}| und \verb|\Datum{}| übersetzt werden würden (Stand: 01.10.2025). Benanntes Tool zeigt sich zudem inkonsistent. Beispielsweise wird \verb|\section{saw}| zu \verb|\Abschnitt{Säge}| übersetzt und \verb|\section{Introduction}| zu \verb|\section{Einführung}| übersetzt.\\\noindent
% reviewed: 1
\subparagraph*{Formen}% Formen bedeutet: Welche Befehle könnten dieses Problem auslösen?
Diese Art von Fehler könnte für jegliche Art von \TeX{}-Befehl auftreten, welcher nicht durch 




\paragraph*{Whitespace}% "Vermisst" meint eigentlich nur, dass man nicht davon ausgehen soll, dass Wörter _nie_ freistehen _könnten_.
\subparagraph*{Beispiel}
...
\paragraph*{Fehlende Dateien}% Könnte als Resultat von Vorigem gedeutet werden, meint jedoch die Inhalte der TeX-Dateien selbst 
\subparagraph*{Beispiel}
\subparagraph*{Beschreibung}
...
\paragraph*{In welcher Umgebung befinden wir uns?}% Wie kann man mit Makros schnell mal etwas verstecken? Wie kann es unklar werden welche Sprache übersetzt werden soll? 
\subparagraph*{Beispiel}
\subparagraph*{Beschreibung}
...
\paragraph*{Mathematische Verwirrung}% Wie sieht es bei rapiden Wechseln zwischen mathematischen Formeln und Textfläche aus? Hier kommen noch Teile der speziellen Probleme mit hinzu; Namentlich: Hohe Vernestungsgrade und "Zwischen verschiedenen Umgebungen"
\subparagraph*{Beispiel}
\subparagraph*{Beschreibung}
...
\paragraph*{Tabellen}% Wurde bereits in der Einleitung kurz aufgegriffen. <-- Beispiel aus Einleitung habe ich als leere Dose neben mir stehen.
\subparagraph*{Beispiel}
\subparagraph*{Beschreibung}
...
%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%% Oben ist Chaos, aber das ist wichtig. Untiges ist an sich nur nice to have.
%%%%%%%%%%%%%%%
